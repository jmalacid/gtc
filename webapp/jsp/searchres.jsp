...
<table cellspacing="3" WIDTH="800px" border="0" >
	<tr>
		...
		--%>	
		<td class="rescab" rowspan="2" align="center" nowrap>
			Prod ID
		</td>
		<td class="rescab" rowspan="2" align="center" nowrap>
			Program ID
		...
		<td class="rescab" rowspan="2" align="center">
			Obs. Mode
		</td>
		<td class="rescab" rowspan="2" align="center" nowrap>
			Init.Time
		...
		<td class="rescab" rowspan="2" style="background-color: #DF7401" align="center" nowrap>
			Pub
		</td>
		<td class="rescab" colspan="8" style="background-color: #4B8A08" align="center" nowrap>
			Reduced Data
		</td>
		<td class="rescab" colspan="9" style="background-color: #0174DF" align="center" nowrap>
			Raw Data
		</td>
		<td class="rescab" rowspan="2" colspan="2" style="background-color:#FA5858" align="center" nowrap>
			External Archives
		</td>
		</tr>
		
		<tr>
		<td class="rescab" style="background-color: #4B8A08" colspan="3" align="center" nowrap>
		...
		<td class="rescab" style="background-color:#4B8A08" colspan="3" align="center" nowrap>
			GTC Reduced Data <span class="form_text" onmouseover="TagToTip('gtcRed', CLICKSTICKY, true, CLICKCLOSE, false, CLOSEBTN, true);"><sup>?</sup></span>
					<span id="gtcRed"> Data reduced as described in Cortés-Contreras et al. (<a href="https://ui.adsabs.harvard.edu/abs/2020MNRAS.491..129C/abstract">2020MNRAS.491..129C</a>) </span><input name="markRedS" type="checkbox" onClick="mark_redS(document.form_fetch_multiple)"></input>
		</td>
		<td class="rescab" style="background-color:#4B8A08" colspan="2" align="center" nowrap>
		...
		<td class="rescab" style="background-color:#0174DF" colspan="3" align="center" nowrap>
			Raw Data <input name="markRaw" type="checkbox" onClick="mark_raw(document.form_fetch_multiple)"></input>
		</td>
		<td class="rescab" style="background-color:#0174DF" colspan="2" align="center" nowrap>
...
while( (resultado=resultados.getNext())!=null ){
	...
	<td class="resfield" nowrap align="left" id="prog" 
		onmouseover="Tip('<%=resultado.getProgName()%>')" 
		onmouseout="UnTip()"><!-- Obs Mode -->
		<%=resultado.getProgId() %><font color="blue">?</font>
	</td>
			...
			if(resultado.getInsName().equalsIgnoreCase("HIPERCAM")){
				...
				if(resultado.getCountRed()>0){ 
					...
					if(red_users>0){
						...
						<td class="resfield" nowrap align="center"><!-- View Reduc. Pub -->
								<%-- <a target="_blank" href="http://adsabs.harvard.edu/abs/<%=resultado.getBibcode().trim()%>">ADS</a> --%>
								<a target="_blank" href="http://adsabs.harvard.edu/abs/<%=bibcode%>">ADS</a>
						</td>
						<td class="resfield" nowrap align="center"><!-- View Reduc. Files -->
								<%-- <a href="viewprod.jsp?prod_id=<%=prod_id%>&modtype=RED&bib=<%=resultado.getBibcode().trim().replaceAll("&", "%26")%>">View</a> --%>
								<a href="viewprod.jsp?prod_id=<%=prod_id%>&modtype=RED&bib=<%=bibcode.trim().replaceAll("&", "%26")%>">View</a>
						</td>
						<td class="resfield" nowrap align="center"><!-- Fetch Reduc. Files -->
						<table>
							<tr>
								<td  class="resfield" nowrap>
									<%-- <a href="fetchDataBlock.jsp?datablock=<%=resultado.getProgId()%>__<%=resultado.getOblId() %>__<%=resultado.getProdId()%>__ALLRED&bib=<%=resultado.getBibcode().trim().replaceAll("&", "%26")%>">Fetch</a> --%>
									<a href="fetchDataBlock.jsp?datablock=<%=resultado.getProgId()%>__<%=resultado.getOblId() %>__<%=resultado.getProdId()%>__ALLRED&bib=<%=bibcode.trim().replaceAll("&", "%26")%>">Fetch</a>
								</td>
								<td>
									<input name="datablock" value="<%=resultado.getProgId()%>__<%=resultado.getOblId() %>__<%=resultado.getProdId()%>__ALLRED" type="checkbox">
								</td>
					...
					String urlAladin70 = "http://gtc.sdc.cab.inta-csic.es/gtc/jsp/aladinjnlp.jsp?prod_id="+prod_id;
					%>
					<td class="resfield" nowrap align="center"> </td>
					<td class="resfield" nowrap align="center"> </td>
					<td class="resfield" nowrap align="center"> </td><!-- QLA -->
...
//No queremos mostrar los reducidos Herve, ponemos que hay 0
				...
				//if(resultado.getCountRed()>0){
					...
					<td class="resfield" nowrap align="center"><!-- View Reduc. Pub -->
							<%-- <a target="_blank" href="http://adsabs.harvard.edu/abs/<%=resultado.getBibcode().trim()%>">ADS</a> --%>
							<a target="_blank" href="http://adsabs.harvard.edu/abs/<%=bibcode.trim()%>">ADS</a>
					</td>
					<td class="resfield" nowrap align="center"><!-- View Reduc. Files -->
							<%-- <a href="viewprod.jsp?prod_id=<%=prod_id%>&modtype=RED&bib=<%=resultado.getBibcode().trim().replaceAll("&", "%26")%>">View</a> --%>
							<a href="viewprod.jsp?prod_id=<%=prod_id%>&modtype=RED&bib=<%=bibcode.trim().replaceAll("&", "%26")%>">View</a>
				...
				%>
					...
					<td class="resfield" nowrap align="center"><!-- Preview Reduc. Files -->
				...
				<%}else if((red_her)>1){
				//}else if((resultado.getCountHer())>1){%>
					<td class="resfield" nowrap align="center"><!-- View Reduc. Files -->
				...
				<%} 
 //Datos reducidos megara QLA
 				if((red_meg)>0){
 					
 					if(resultado.getInsName().equals("HORuS")){
 						%>
						<td class="resfield" nowrap align="center"><!-- View Reduc. Files -->
								<a target="_blank" href="http://gtc.sdc.cab.inta-csic.es/gtc/data/2000X/<%=resultado.getProgId() %>/OB<%=resultado.getOblId().toLowerCase()%>/chain/n<%=resultado.getFileName().replaceAll(".fits.gz", ".png")%>">View</a>
						</td>
						<td class="resfield" nowrap align="center"><!-- Fetch Reduc. Files -->
						<table>
							<tr>
				...
				<%
	 				...
	 				//if(resultado.getCountMeg()>1){%>
						<td class="resfield" nowrap align="center"><!-- View Reduc. Files -->
								<a href="viewprod.jsp?prod_id=<%=prod_id%>&modtype=MEG">View</a>
						</td>
						<td class="resfield" nowrap align="center"><!-- Fetch Reduc. Files -->
						<table>
							<tr>
				...
				<td class="resfield" nowrap align="center"><!--Preview ReducS. Files -->
				</td>
				<td class="resfield" nowrap align="center"><!--Fetch ReducS. Files -->
				</td> 
				<td class="resfield" nowrap align="center"><!-- QLA -->
				</td>
				<td class="resfield" nowrap align="center"><!-- QLA -->
				</td> 
		...
		<%}//Fin de else instrumento
			...
			<td class="resfield" nowrap align="center" colspan="17"> <font color="red">Private Data. They will become public on: <%=resultado.getProdPrivate() %></font></td>
		<%}
		//NED
