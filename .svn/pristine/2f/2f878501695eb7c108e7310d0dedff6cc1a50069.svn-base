...
public class LogProgram implements Comparable<LogProgram>{
	private String name;
	private HashMap<String, LogOblock> oblocks = new HashMap<String, LogOblock>();
	private LogError[] errores = new LogError[0];
	
	public LogProgram(String logLine) throws Exception{
		String pattern = "^([\\w-]+)[^:]+:\\s(.+)*$";
		
		if(!logLine.matches(pattern)) throw new Exception("La línea del fichero de log no tiene formato adecuado para programa: "+logLine);
	...
	public LogOblock[] getOblocks(){
		Iterator<LogOblock> it =  oblocks.values().iterator();
		Vector<LogOblock> aux = new Vector<LogOblock>();
		while(it.hasNext()){
			aux.add(it.next());
		}
		LogOblock[] salida = (LogOblock[])aux.toArray(new LogOblock[0]);
		Arrays.sort(salida);
		return salida;
	}
	...
	public LogOblock getOblock(String name){
		return oblocks.get(name);
	}

	public void addOblock(LogOblock oblock){
		oblocks.put(oblock.getName(), oblock);
	}
	...
	public int compareTo(LogProgram o) {
