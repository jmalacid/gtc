...
public class ErrorDb {
	...
	public ErrorDb(ResultSet resset) throws SQLException{
	 ...
	 * @return
	 */
	public static ErrorDb[] getErrors(String mensaje){
		String[] errIds = mensaje.split(";");
		Vector<ErrorDb> aux = new Vector<ErrorDb>();
		for(int err=0; err<errIds.length; err++){
			ErrorDb error=new ErrorDb();
			//System.out.println(errIds[err]+" : "+extraeId(errIds[err]));
			error.setErr_id(extraeId(errIds[err]));
			if(error.getErr_id().startsWith("E-")) aux.add(error);
		}
		
		return (ErrorDb[])aux.toArray(new ErrorDb[0]);
	}
